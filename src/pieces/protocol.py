...
class PeerConnection:
    ...
    def __init__(self, queue: Queue, info_hash, peer_id, piece_manager, on_block_cb=None):
        ...
        :param piece_manager: The manager responsible to determine which pieces to request
    ...
    async def _request_piece(self):
        ...
        if block:
            ...
            logging.debug('Requesting block {block} for piece {piece} of {length} bytes from peer {peer}'
                    .format(piece=block.piece, block=block.offset, length=block.length, peer=self.remote_id))
...
class PeerMessage:
    ...
    NOTE: The Handshake messageis different in layout compared to the other messages.
...
class Interested(PeerMessage):
    ...
    def encode(self) -> bytes:
        ...
        message (ready to be transmitted).
        """
        return struct.pack('>Ib', 1,  # Message length
...
class Have(PeerMessage):
    ...
    def encode(self):
        return struct.pack('>IbI', 5, # Message length
                            PeerMessage.Have, self.index)
...
class Piece(PeerMessage):
    ...
    def encode(self):
        ...
        return struct.pack('>IbII' + str(len(self.block)) + 's', message_length,
