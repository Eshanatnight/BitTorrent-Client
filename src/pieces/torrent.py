...
# represents the file within the torrent
TorrentFile = namedtuple('TorrentFile', ['path', 'length'])

class Torrent:
    """
    Represents the torrent metadata within a .torrent file

    This basically acts as a wrapper around the bencoded data with
    ...
    def __str__(self):
        return 'Filename: {0}\n File length: {1}\n' \
            'Announce URL: {2}\n Hash: {3}'.format(self.meta_info[b'info'][b'name'],
                            self.meta_info[b'info'][b'length'],
                            self.meta_info[b'announce'], self.info_hash)
