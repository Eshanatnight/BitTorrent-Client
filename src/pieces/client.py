...
class TorrentClient:
    ...
    async def start(self):
        ...
        self.peers = [PeerConnection(self.available_peers, self.tracker.torrent.info_hash,
                        self.tracker.peer_id, self.piece_manager, self._on_block_retrieved)
...
class Piece:
    ...
    """
    def __init__(self, index: int, blocks: List[T], hash_value):
        self.index = index
        self.blocks = blocks
...
class PieceManager:
    ...
    def block_received(self, peer_id, piece_index, block_offset, data):
        ...
        """
        logging.debug('Received block {block_offset} for piece {piece_index} from peer {peer_id}: '
            .format(block_offset=block_offset, piece_index=piece_index, peer_id=peer_id))
            ...
            if piece.is_complete():
                if piece.is_hash_matching():
                    ...
                    logging.info(
                        ...
                        .format(complete=complete, total=self.total_pieces, per=(complete/self.total_pieces)*100))
                else:
                    logging.info('Discarding corrupt piece {index}'.format(index=piece.index))
                    piece.reset()
        else:
